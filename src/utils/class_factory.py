...
class ClassFactory:
    ...
    def get_cls(cls, module_type, class_name=None):
        ...
        if not cls.is_exist(module_type, class_name):
            raise ValueError(f"Can't find type `{module_type}` and name"
                             f" `{class_name}` in class registry.")
        if not class_name:
    ...
    def get_instance_from_cfg(cls, cfg, module_type=ModuleType.GENERAL, default_args=None):
        ...
        if isinstance(obj_type, str):
            ...
            raise ValueError(f"Can't find class type {type} which "
                             f"name `{obj_type}` in class registry.")
    ...
    def get_instance(cls, module_type=ModuleType.GENERAL, obj_type=None, args=None):
        ...
        if isinstance(obj_type, str):
            ...
            raise ValueError(f"Can't find class type {type} which name"
                             f" `{obj_type}` in class registry.")
